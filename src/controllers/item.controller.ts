...
export const createItem = async (req: Request, res: Response) => {
  try {
    ...
    const taxType = salesTax === "With Tax" ? "with_tax" : "without_tax";
    if (!name || !itemType) {
      return res
        .status(400)
  ...
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        lowStockQty:
          product.lowStockQty != null ? String(product.lowStockQty) : "-",
  ...
  try {
    ...
    if (!godownId || !type || qty === undefined) {
      return res
        ...
        .json({
          success: false,
          message: "godownId, type and qty are required",
        });
    }
