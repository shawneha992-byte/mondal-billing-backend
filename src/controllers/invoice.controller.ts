...
export const createInvoice = async (req: Request, res: Response) => {
  const {
    ...
    applyTcs           = false,
    autoRoundOff       = false,
    // Pre-computed totals from frontend (trusted — frontend mirrors our formula)
  ...
  try {
    const invoice = await prisma.$transaction(async (tx) => {
      ...
      // ── 2. Totals — use frontend pre-computed values (they mirror our formula) ──
      // Fall back to server-side calculation if frontend didn't send them.
      const additionalChargesTotal: number = payloadChargesTotal
        ?? additionalCharges.reduce((sum: number, c: { name: string; amount: number }) => sum + (c.amount ?? 0), 0);
      ...
      // ── 3. Invoice number (safe: uses MAX numeric suffix, not count) ──────
      // Sort by id desc (always reliable) to get the latest invoice
      const lastInvoice = await tx.invoice.findFirst({
        orderBy: { id: "desc" },
        select:  { invoiceNo: true },
      });
      // Extract highest numeric suffix across ALL invoices to handle deletions/gaps
      ...
      let invoiceNo = `INV-${String(nextSeq).padStart(5, "0")}`;
      while (await tx.invoice.findUnique({ where: { invoiceNo } })) {
        nextSeq++;
        invoiceNo = `INV-${String(nextSeq).padStart(5, "0")}`;
      }
          ...
          items: {
            create: items.map((item: any) => {
              ...
              return {
                ...
                taxAmount: lineTax,
                total:     lineTaxable,   // taxable line amount (excl. tax)
              };
            }),
          },
  ...
  if (stock) {
    await tx.productStock.update({
      where: { id: stock.id },
      data: {
        openingStock: { decrement: item.quantity }
      }
    });
  }
}
