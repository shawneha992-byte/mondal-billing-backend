...
export const getPartyLedger = async (req: Request, res: Response) => {
  try {
    ...
    const openingBalance =
      party.openingBalanceType === "To_Collect"
    ...
    const invoiceIds = entries.filter(e => e.refType === "Invoice" && e.refId).map(e => e.refId as number);
    const paymentIds = entries.filter(e => e.refType === "Payment" && e.refId).map(e => e.refId as number);
    const returnIds  = entries.filter(e => e.refType === "Return"  && e.refId).map(e => e.refId as number);
    ...
    const [invoices, payments, returns] = await Promise.all([
      invoiceIds.length > 0
        ...
        : [],
      paymentIds.length > 0
        ? prisma.paymentIn.findMany({ where: { id: { in: paymentIds } }, select: { id: true, paymentNo: true } })
        : [],
      returnIds.length > 0
        ? prisma.salesReturn.findMany({ where: { id: { in: returnIds } }, select: { id: true } })
        : [],
    ]);

    const invoiceMap = new Map((invoices as any[]).map(i => [i.id, i.invoiceNo]));
    const paymentMap = new Map((payments as any[]).map(p => [p.id, p.paymentNo]));
    ...
    const data = entries.map((e, idx) => {
      ...
      if (e.refType === "Invoice" && e.refId) {
        voucher = (invoiceMap.get(e.refId) as string) || `INV-${e.refId}`;
      } else if (e.refType === "Payment" && e.refId) {
        voucher = (paymentMap.get(e.refId) as string) || `PAY-${e.refId}`;
...
export const getPartyBalance = async (req: Request, res: Response) => {
  try {
    const partyId = Number(req.params.id);
    const latest  = await prisma.partyLedger.findFirst({
      where:   { partyId },
      orderBy: { date: "desc" },
    });
